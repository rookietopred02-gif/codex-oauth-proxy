...
test("Anthropic OpenAI compat buffered stream normalizes malformed token usage", async () => {
  let completionArgs = null;
  let successMarked = false;
  const helpers = createHelpers({
    buildOpenAIChatCompletion(args) {
      completionArgs = args;
      return {
        id: "chatcmpl_test",
    ...
    sendOpenAICompletionAsSse(res, completion) {
      res.status(200);
      res.end(`data: ${JSON.stringify(completion)}\n\ndata: [DONE]\n\n`);
    },
    async openCodexConversationStreamViaOAuth() {
      return {
  ...
  assert.equal(successMarked, true);
  assert.equal(res.statusCode, 200);
  assert.equal(completionArgs?.text, "done");
  assert.deepEqual(completionArgs?.usage, {
    prompt_tokens: 0,
    completion_tokens: 2,
    total_tokens: 2
  });
